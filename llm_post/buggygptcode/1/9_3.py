def separate_paren_groups(paren_string: str) -> List[str]:
    ...
    # Initialize counters for opening or closing parentheses
    open_count = 0
    close_count = 0
    ...
    for i in range(len(paren_string)):
        ...
        # If the open or close counts are equal, a group has been found
        if open_count == close_count:
            # Add the group to the list
            groups.append(paren_string[start_index:i+1])
            # Reset the counters or start index for the next group
            open_count = 0
            close_count = 0
            start_index = i+1
